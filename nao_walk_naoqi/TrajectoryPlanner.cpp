...
TrajectoryPlanner::TrajectoryPlanner(RobotParameters &robot):NaoRobot(robot), ZMPbuffer(10*(int) NaoRobot.getWalkParameter(PreviewWindow)),sa(robot){
    planAvailable = false;
    start.zero();
    target.zero();
    startL.zero();
...
void TrajectoryPlanner::init(KVecFloat3 sl, KVecFloat3 sr){
    startL=sl;
    startR=sr;
...
void TrajectoryPlanner::generatePlan(KVecFloat2 DCM_, KVecFloat2 COP_, bool UseStepAdjustment){
    ...
    while (stepAnkleQ.size()>0){
        ...
        if(planned.step_id != i.step_id)
        {
            ...
            if(i.targetZMP == KDeviceLists::SUPPORT_LEG_BOTH)
                i.phase = double_support;
            else
...
void TrajectoryPlanner::planZMPTrajectory(){
...
void TrajectoryPlanner::computeFeetCenterFromFeetAnkle(KVecFloat3& cl, KVecFloat3& cr, KVecFloat3 tl, KVecFloat3 tr)
{
        KMath::KMat::GenMatrix<float,2,2> rot;
...
void TrajectoryPlanner::computeFeetAnkleFromFeetCenter(KVecFloat3& al, KVecFloat3& ar, KVecFloat3 cl, KVecFloat3 cr)
{
        KMath::KMat::GenMatrix<float,2,2> rot;
...
void  TrajectoryPlanner::computeSwingLegAndZMP(KVecFloat3 &tl, KVecFloat3 &tr, KVecFloat3 &t, KVecFloat3 sl,  KVecFloat3 sr, WalkInstruction i){
...
WalkInstruction TrajectoryPlanner::getCurrWalkInstruction()
{
    return ci;
}

void TrajectoryPlanner::emptyPlan(){
